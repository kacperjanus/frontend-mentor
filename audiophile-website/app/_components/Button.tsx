...
function Button({type, children}: {type: string, children?: ReactNode}) {
    ...
    if(type === "secondary") return <button className="w-40 h-12 bg-transparent border-[1px] border-solid border-black hover:bg-black group"><span className="uppercase text-[13px] group-hover:text-white font-bold text-black">{children}</span></button>
