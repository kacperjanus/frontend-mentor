...
function MultipleSelectOption({title, description, primary, index, setValues, values, stepNumber} : MultipleSelectOptionProps) {
    ...
    return (
        <div onClick={handleCheck} className={`${isChecked ? "bg-magnolia border-marine-blue" : "border-light-gray"} cursor-pointer hover:border-marine-blue border-[1px] border-solid rounded-lg py-4 px-4 md:px-5 flex items-center justify-between`}>
            <div className="relative flex gap-4 md:gap-8 items-center text-xs md:text-base">
                <input type="checkbox" className="peer cursor-pointer appearance-none w-5 h-5 border-[1px] border-light-gray rounded-md bg-white checked:bg-purplish-blue checked:border-none" checked={isChecked} readOnly />
                <img className="absolute w-3 h-3 peer-checked:block hidden ml-1" src="/icon-checkmark.svg" alt="Checkmark icon"/>
                <div>
