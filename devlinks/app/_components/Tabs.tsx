...
export default function Tabs({labels, icons}: { labels: string[], icons: string[] }) {
    ...
    return <div className="flex">
        {labels.map((label, i) =>
            <div key={label} onClick={() => setActive(i)} className={`flex gap-2 px-[27px] py-[11px] rounded-lg heading-s hover:text-dark-lavender cursor-pointer ${active === i ? "bg-lavender text-dark-lavender" : "text-gray"}`}>
